      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-4">
        <div>
          <nav className="flex items-center space-x-2 text-on-surface-variant text-[9px] font-black uppercase tracking-[0.2em] mb-3">
            ...
            <span className="text-primary">Quản lý Chuyến thám hiểm</span>
          </nav>
          <h1 className="text-3xl font-black text-on-surface tracking-tight leading-none">Danh bạ Điểm đến</h1>
          <p className="text-slate-400 text-sm leading-relaxed font-medium mt-2">Quản lý kho tàng cảnh quan và các hành trình được tuyển chọn bậc nhất.</p>
        </div>
        <Link to="/admin/tours/new" className="signature-gradient text-white font-bold text-xs px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2 scale-100 active:scale-95 whitespace-nowrap">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-4">
              <h3 className="text-lg font-black tracking-tight text-on-surface">Bộ sưu tập Tinh hoa Việt</h3>
              <div className="h-4 w-px bg-slate-200"></div>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">5 khu vực</span>
            </div>
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                <div key={dest.id} className="group bg-surface-container-lowest rounded-2xl overflow-hidden border border-surface-container-low transition-all hover:border-primary/30">
                  <div className="relative aspect-[4/3] overflow-hidden">
                    <img 
                      ...
                      src={dest.image} 
                    />
                    <div className="absolute top-3 left-3 bg-white/90 backdrop-blur px-2.5 py-1 rounded-lg text-[9px] font-black text-primary uppercase tracking-widest">
                      {dest.region}
                    </div>
                  </div>
                  <div className="p-5">
                    ...
                    <div className="flex gap-2 pt-4 border-t border-surface-container-low">
                      <Link to="/admin/tours/new" className="flex-1 bg-surface-container-low hover:bg-primary/5 hover:text-primary text-slate-600 font-bold py-2 rounded-xl transition-all text-[10px] uppercase tracking-widest flex items-center justify-center gap-1.5" title="Thêm tour mới">
                        <span className="material-symbols-outlined text-xs">add</span>
                        Thêm
                      </Link>
                      <Link to={`/admin/tours/edit/${dest.id}`} className="flex-1 bg-surface-container-low hover:bg-primary/5 hover:text-primary text-slate-600 font-bold py-2 rounded-xl transition-all text-[10px] uppercase tracking-widest flex items-center justify-center gap-1.5" title="Chỉnh sửa Tour">
                        <span className="material-symbols-outlined text-xs">edit_note</span>
                        Sửa
                      </Link>
                      <button className="w-10 bg-error/5 hover:bg-error text-error hover:text-white rounded-xl transition-all flex items-center justify-center group/del text-[10px]">
              ...
              <Link to="/admin/tours/new" className="group border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center p-8 hover:border-primary/40 hover:bg-primary/[0.02] transition-all cursor-pointer">
                ...
                <p className="text-xs font-black uppercase tracking-widest text-slate-400 group-hover:text-primary transition-colors">Tạo khu vực mới</p>
              </Link>
            </div>
          ) : (
                    ...
                    <tr key={dest.id} className="hover:bg-primary/[0.01] transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <img alt={dest.name} className="w-10 h-10 rounded-lg object-cover grayscale group-hover:grayscale-0 transition-all" src={dest.image} />
                          <span className="font-bold text-slate-700">{dest.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{dest.region}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className="font-black text-sm text-slate-800">{dest.toursCount}</span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <Link to={`/admin/tours/edit/${dest.id}`} className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-all" title="Chỉnh sửa">
                             <span className="material-symbols-outlined text-sm">edit</span>
                          </Link>
                          <button className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-error hover:bg-error/5 rounded-lg transition-all" title="Xóa">
